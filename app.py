...
def show_report():
    ...
    try:
        ...
        fig, ax = plt.subplots()
        ax.scatter(data["Entropy"], data["Dictionary Size"], label="Tamanho do Dicionário", marker="o", linestyle="-")
        ax.set_xlabel("Entropia")
        ax.set_ylabel("Tamanho do Dicionário")
        ax.set_title("Tamanho do Dicionário em Função da Entropia")
        ax.legend()
        st.pyplot(fig)
        st.markdown(""" No caso estático, podemos ver que o tamanho do dicionário criado costuma ser de 4096 bytes""")
        ...
        st.markdown("""Aqui, podemos ver que a taxa de compressão não é sensível à entrada: se observarmos o eixo y de todos os gráficos para os níveis diferentes de entropia, o valor da taxa de compressão pouco muda.""")
        # Gráfico Interativo
...
def show_dynamic_report():
    ...
    st.markdown("""
    ## Testes e Gráficos
    Nesta seção, apresentamos os dados coletados a partir dos testes automáticos do algoritmo de compressão LZW Dinâmico, usando as mesmas métricas de avaliação anteriores. Algumas poucas análises mudaram e serão descritas:
    """)
    ...
    try:
        ...
        ax.legend()
        st.pyplot(fig)
        st.markdown("""Comparado ao outro algoritmo, aqui vemos que os tamanhos de dicionário variam mais, justamente pela sua natureza dinâmica.""")
...
elif page == "Testes do LZW Padrão":
    show_report()
elif page == "Testes do LZW Dinâmico":
    show_dynamic_report()
